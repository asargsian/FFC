...
train = pd.read_csv('/Users/ann/Desktop/FF/FFChallenge_v2/train.csv');
train = train[['challengeID', 'gpa']]
train = train.dropna()
training = pd.concat([background_filled_knn.set_index('challengeID'),train.set_index('challengeID')], axis=1, join='inner');


#getting features from rlasso with tuning
feature_set = []
from sklearn.linear_model import RandomizedLasso
params = [0.0000004, 0.00000004]
...
intrs = list(set(feature_set[0]).intersection(*feature_set[1:]))

#using Random Forest for prediction
from sklearn.ensemble import RandomForestRegressor
i=200
while True:
    predictors = training[intrs]
    targets = training['gpa']
    ...
    i+=1
    if score >=0.3:
        all_pred = reg.predict(background[intrs])
        pd.DataFrame(all_pred).to_csv('intersection_pred_abv_score'+str(score)[:10]+'.csv')
    print(i, score)
...
for i in gpas:
    if i < 2.75:
        training['gpa'][training['gpa']==i] = 'Low'
    elif i < 3.25 and i >= 2.75:
        training['gpa'][training['gpa']==i] = 'Mid'
    elif i <= 4 and i >= 3.15:
        training['gpa'][training['gpa']==i] = 'Top'
...
km.fit(expmat)
km.labels_.to_csv('km_labels.csv')
...
labels = pd.read_csv('km_labels.csv')['0'].tolist()

train = pk.load(open('training_datamatrix_andfeaturenames.pkl'))
...
for tmp in [[0, 2], [1], [3], [4]]:
    ...
    sigcoefs = data_all.columns[coefs.pvalues < .05][inds]
